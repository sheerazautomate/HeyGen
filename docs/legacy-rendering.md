> **Archived operator documentation.** The PAT-based browser composer described here has been replaced
> by the public pilot studio. Manual/CLI rendering commands remain available only for trusted operators.
> Do not feed untrusted public submissions into this legacy workflow; use `pilot-render.yml`.
